...
```
make test                            # build + run unit tests (default target)
make unit-tests                      # run unit tests only
make build-examples                  # compile examples only
make config=debug                    # debug build
make clean                           # clean build artifacts
make lint                            # run pony-lint over the repo
```

No external dependencies — pure Pony stdlib only.

## Conventions

- `_Unreachable()` (`_unreachable.pony`) for impossible code paths.
